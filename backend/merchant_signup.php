...
require '../vendor/autoload.php';
  ...
  if(isset($_POST['fname']) && $_POST['fname'] != "")
  {
    ...
    $state = $_POST['state'];
    $company_id = "automallMerchant". str_shuffle(substr(md5(time().mt_rand().time()), 0,20));
      ...
      if($picture == "success")
      {
        ...
        if(send_mail($email, $fname, $lname, $token))
        {
          ...
          if(mysqli_execute($save_token))
          {
            ...
            $stmt = "INSERT INTO am_merchant(merchant_id, fname, lname, company_name, email, password, phone, address, state, logo, date_created, date_updated) VALUES(?,?,?,?,?,?,?,?,?,?,NOW(),NOW())";
            $new_company = mysqli_prepare($con, $stmt);
            mysqli_stmt_bind_param($new_company, 'ssssssssis', $company_id, $fname, $lname, $company, $email, $pass, $phone, $address, $state, $picture);
            if(mysqli_execute($new_company))
            {
              $res =  "success";
            }
            else
...
function send_mail($email, $fname, $lname, $token){
        ...
        <a href="'.$web_link.'verify.php?email='.$email.'&token='.$token.'&type=merchant" style = "color: #fff;">Activate Account</a>
        </div>
        </body>
        </html>';
