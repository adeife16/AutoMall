  ...
  else
  {
    ...
    if($user_type == "company")
    {
      ...
      if($result)
      {
        ...
        $stmt2 = "SELECT s.*, r.first_name, r.last_name, r.picture FROM rating s, rider r WHERE s.rater_id = r.rider_id AND s.user_id = ?";
    ...
    // profile details for rider
    elseif($user_type == "rider")
    {

      $stmt = "SELECT r.*, l.lga_name, s.state_name FROM rider r, local_governments l, states s WHERE r.city = l.id AND r.state = s.id AND rider_id = ?";
      $get_stmt = mysqli_prepare($con, $stmt);
      mysqli_stmt_bind_param($get_stmt, 's', $user_id);
      mysqli_execute($get_stmt);
      $result = mysqli_stmt_get_result($get_stmt);
      if($result)
      {
        ...
        // employment details for rider
        $stmt = "SELECT * FROM employment WHERE rider_id = ? AND company_id = ?";
        $get_stmt = mysqli_prepare($con, $stmt);
        mysqli_stmt_bind_param($get_stmt, 'ss', $user_id, $session_id);
        mysqli_execute($get_stmt);
        $result = mysqli_stmt_get_result($get_stmt);
        if($result)
        {
          if(mysqli_num_rows($result) == 1)
          {
            ...
            $request = array("employ_request" => "sent", 'employ_status' => $status);
          }
          else
          {
            $request = array("employ_request" => "not sent", 'employ_status' => null);
        ...
        // rating details for rider
        // $result = mysqli_query($con,"SELECT s.*, r.first_name, r.last_name, r.picture FROM rating s, rider r WHERE s.rater_id = r.rider_id AND s.user_id = '$user_id' ");

        $stmt = "SELECT s.*, c.company_name, c.logo FROM rating s, company c WHERE s.rater_id = c.company_id AND s.user_id = ?";
        $get_stmt = mysqli_prepare($con, $stmt);
        mysqli_stmt_bind_param($get_stmt, 's', $user_id);
        mysqli_execute($get_stmt);
        $result = mysqli_stmt_get_result($get_stmt);
        if($result)
        {
          if(mysqli_num_rows($result) > 0)
          {
            ...
            $stmt = "SELECT * FROM rating WHERE user_id = ? AND rater_id = ?";
            $get_stmt = mysqli_prepare($con, $stmt);
            mysqli_stmt_bind_param($get_stmt, 'ss', $user_id, $session_id);
            mysqli_execute($get_stmt);
            $result = mysqli_stmt_get_result($get_stmt);
...
if(isset($_POST['employRequest']) && $_POST['employRequest'] != "")
{
  ...
  $stmt = "SELECT r.first_name, r.last_name, c.company_name from rider r, company c WHERE r.rider_id = ? AND c.company_id = ? ";
  ...
  if($result)
  {
    ...
    $stmt = "INSERT INTO employment(rider_id, company_id, rider_name, company_name, date_issued) VALUES(?,?,?,?,NOW())";
    $insert_stmt = mysqli_prepare($con, $stmt);
    mysqli_stmt_bind_param($insert_stmt, "ssss", $user, $session_id, $rider_name, $company_name);
...
if(isset($_POST['cancelRequest']) && $_POST['cancelRequest'] != "")
{
  ...
  $stmt = "DELETE FROM employment WHERE rider_id = ? AND company_id = '$session_id'";
